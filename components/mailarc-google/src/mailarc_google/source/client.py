...
class GmailClient:
    ...
    async def get(
        ...
        connection, a body that is not JSON — leaves as one of the three
        ...
        if response.status_code != httpx.codes.OK:
            raise self._refusal(path, response)
    ...
    def _refusal(self, path: str, response: httpx.Response) -> MailError:
        ...
        # 404 and every other refusal: this one call will never succeed, but
        # the account is fine and the import goes on without that message.
