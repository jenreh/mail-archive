...
768 and not 1536 because only 768 is reachable from both providers, and the
reason is an asymmetry rather than a preference: OpenAI's
``text-embedding-3-small`` is 1536 natively but can be *asked* for 768 through
its ``dimensions`` parameter — which the adapter always does, for exactly this
reason — while ``nomic-embed-text`` is 768 natively and has **no such
parameter**, so 768 is the only length the local path can ever answer. Raising
this to 1536 therefore buys OpenAI nothing it does not already have and costs
the no-account path entirely, on top of doubling the memory (measured 14 KB per
message against 7.3 KB).

An archive that wants OpenAI's native 1536 is not stuck with this literal:
:func:`mailarc_analytics.semantic.indexing.rebuild_index` resizes the live index
from the settings page and clears the vectors that were the old length. This is
the length a *fresh* archive starts at, chosen so that the provider needing no
account works without one.
"""
